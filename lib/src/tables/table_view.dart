...
class TableViewState extends State<TableView> with AutomaticKeepAliveClientMixin {
  ...
  void didChangeDependencies() {
    if (widget.table == tableUser) {
      const sysClause = "$columnName != 'sys'";
      if (whereClause != null) {
        whereClause = '$sysClause and $whereClause';
      } else {
        whereClause = sysClause;
      }
    }
  ...
  List<DataCell> _rowCells(dynamic item) {
    ...
    if (item is User) {
        cells = [
          ...
          DataCell(Text(item.einstellungen)),
        ];
